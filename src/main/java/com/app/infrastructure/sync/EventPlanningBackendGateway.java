...
public class EventPlanningBackendGateway {

    private final ConfigProvider configProvider;

    public EventPlanningBackendGateway(ConfigProvider configProvider, AuthenticatedHttpClient authenticatedHttpClient) {
        this.configProvider = Objects.requireNonNull(configProvider);
    }

    public Map<String, EventPlanning> fetchEventPlanningsById() {
        ...
        String sql = "SELECT id::text AS id, \"startDate\", \"endDate\", event_id FROM event_plannings";
        ...
        try (Connection conn = DriverManager.getConnection(jdbcUrl);
            ...
            DailyLogger.logInfo("Sync", "Fetched " + byId.size() + " event plannings from PostgreSQL.");
        } catch (Exception e) {
            DailyLogger.logError("Sync", "Failed to fetch event plannings: " + e.getMessage(), e);
        }
        return byId;
    }
